...
public class ArrayTasks {
    ...
    public int[] getOnlyPositiveNumbers(int[] arr) {
        ...
        int[] onlyPositiveNumbersArray = new int[size];
        int lastIndex = 0;
        for (int i : arr) {
            if (i > 0) {
                onlyPositiveNumbersArray[lastIndex] = i;
                lastIndex++;
            }
        }
        return onlyPositiveNumbersArray;
    }
    ...
    private static void bubbleSort(int[][] arr, int x) {
        for (int i = 0; i < arr[x].length - 1; i++) {
            for (int j = 0; j < arr[x].length - i - 1; j++) {
                if (arr[x][j + 1] < arr[x][j]) {
                    int swap = arr[x][j];
                    arr[x][j] = arr[x][j + 1];
                    arr[x][j + 1] = swap;
                }
